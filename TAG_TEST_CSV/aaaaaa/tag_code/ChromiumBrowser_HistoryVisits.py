...
# =========================================
# 1. D:~Z: 전체에서 ccit 아래 HistoryVisits CSV 찾기
# =========================================

def find_historyvisits_csvs_under_ccit() -> List[Tuple[Path, Optional[datetime]]]:
    """
    D:~Z: 전체를 재귀적으로 돌면서
...
def build_description(row: Dict[str, str]) -> str:
    """
    한 행(row)에서:
      - ID, VisitID, FromVisitID, LastVisitedTime (UTC), LastVisitedTime, SourceFile 은 description에서 제외
...
def tag_historyvisits_csv(input_path: Path,
    ...
    - lastwritetimestemp: 'LastVisitedTime (UTC)'를 그대로 문자열로 사용
    - descrition: ID/VisitID/FromVisitID/SourceFile/LastVisitedTime 계열 제외하고
    ...
    with input_path.open("r", encoding="utf-8-sig", newline="") as f_in, \
        ...
        for row in reader:
            ...
            # lastwritetimestemp 컬럼에 들어갈 문자열 (그대로 사용)
...
def main():
    print("[HistoryVisits] D:~Z: + ccit 경로에서 *_ChromiumBrowser_HistoryVisits_*.csv 탐색 중...")

    candidates = find_historyvisits_csvs_under_ccit()
    if not candidates:
        print("[-] *_ChromiumBrowser_HistoryVisits_*.csv 파일을 찾지 못했습니다.")
        return
