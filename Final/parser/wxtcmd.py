...
MODULE_NAME="WxTCmd"; BIN=Path(r"C:\KAPE\Modules\bin")

def _has()->bool:
    for p in BIN.rglob("*.exe"):
        if p.name.lower()=="wxtcmd.exe": return True
    return False
...
def run(drive_letters:List[str],unmount_callback,cfg:dict)->bool:
    ...
    if not _has(): print("[SKIP] WxTCmd: exe not found"); return False
    for dl in drive_letters:
        art=_loc(BASE_OUT,dl)
        if not art: print(f"[SKIP] {dl} WxTCmd: Artifacts missing"); continue
