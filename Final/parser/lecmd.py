...
def _stream(cmd: list[str], log: Path, timeout: int) -> int:
    ...
    with open(log, "w", encoding="utf-8") as lf:
        ...
        # // [코드 삽입 시작]
        # stdout을 bytes로 받고, 우리가 직접 안전 디코딩(UTF-8 실패 시 CP949/mbcs 등 fallback)
...
def run(drive_letters: List[str], unmount_callback, cfg: dict) -> bool:
    ...
    if not _find_exe("LECmd.exe"):
        print("[SKIP] LECmd: exe not found")
        return False

    for dl in drive_letters:
        art = _loc(BASE_OUT, dl)
        if not art:
            print(f"[SKIP] {dl} LECmd: Artifacts missing")
            continue
