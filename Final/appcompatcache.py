...
def _has()->bool:
    for p in BIN.rglob("*.exe"):
        if p.name.lower()=="appcompatcacheparser.exe": return True
    return False
...
def run(drive_letters:List[str],unmount_callback,cfg:dict)->bool:
    ...
    if not _has(): print("[SKIP] AppCompatCacheParser: exe not found"); return False
    for dl in drive_letters:
        art=_loc(BASE_OUT,dl)
        if not art: print(f"[SKIP] {dl} AppCompat: Artifacts missing"); continue
